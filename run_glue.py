...
def main():
    ...
    if not args.enable_galore:
        ...
        # id_galore_params = map(id, galore_params)
    ...
    peak_alloc = torch.cuda.max_memory_allocated("cuda")
    with open(os.path.join(args.output_dir, "peak_alloc.txt"), "w") as f:
        f.write(f"Peak memory usage - Total size={peak_alloc / (1024) ** 2:.2f}MB\n")
    print(f"Peak memory usage - Total size={peak_alloc / (1024) ** 2:.2f}MB")
