...
class AdamW_Decomposed_LoMo(Optimizer):
    """
    ...
    def __init__(
        ...
        if self.zero3_enabled:  # zero3 is enabled
            raise ValueError("zero3 not supported yet")
        else:
                ...
                if p in param_to_module.keys():
                    ...
                    if module is not None and id(module) not in replaced:
                        ...
                        if '.' in module_name:
                            ...
                            for current_p in path[:-1]:
                                parent = getattr(parent, current_p)
                            setattr(parent, path[-1], new_module)
                        else:
                            setattr(model, module_name, new_module)
                        new_params += [param for param in new_module.parameters() if param.requires_grad]
            ...
            for param in group["params"]:
                if self.zero3_enabled:
                    ...
                    if param.requires_grad:
                        param.register_hook(self.grad_func)
                        ...
                        if p.requires_grad and p.grad is not None:
                            ...
                            if "grad_buffer" in state.keys():
                                ...
                                if "exp_avg" not in state:
                                    ...
                                    state["exp_avg"] = torch.zeros_like(grad_fp32)
                                ...
                                if group["correct_bias"]:  # No bias correction for Bert
                                    ...
                                    bias_correction2 = (1.0 - beta2 ** state["step"])**2
                                    step_size = step_size * bias_correction2 / bias_correction1
                                    #step_size = step_size * bias_correction2
                                p_fp32.addcdiv_(grad_fp32, denom, value=-step_size)
                                ...
                                if "exp_avg" not in state:
                                    ...
                                    state["exp_avg_sq"] = torch.zeros_like(grad_fp32)
