...
class GradBuffer:
    ...
    def update(self, input, grad_output):
        ...
        self.v_out_queue[self.pointer].add_(grad_output.abs().pow(self.p).sum(dim=0), alpha=1-self.beta2)
        self.v_in_queue[self.pointer].add_(input.abs().pow(self.q).sum(dim=0), alpha=1-self.beta2)
    ...
    def data(self):
        ...
        return self.exp_avg, self.v_out_queue.pow(1/self.p).t().mm(self.v_in_queue.pow(1/self.q))
